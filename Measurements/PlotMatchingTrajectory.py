...
def CapacitorTrajectory(Positionsfilepath,FolderLocation):
    Positionsfilepath = Path(Positionsfilepath)
    f = open(Positionsfilepath,'r')
    lines = f.readlines()
    pos = np.zeros(shape=(len(lines),2))
    for i,line in enumerate(lines):
        pos[i,0] = line.split()[5]
        pos[i,1] = line.split()[3]
    ...
    plt.tricontour(pos[len(pos)-len(numbers):,0],pos[len(pos)-len(numbers):,1],Gammas,15,linewidths=0.5,colors='k') #only plot last ones
    plt.tricontourf(pos[len(pos)-len(numbers):,0],pos[len(pos)-len(numbers):,1],Gammas,15) #only plot last ones
    plt.colorbar()
    plt.plot(pos[len(pos)-len(numbers):,0],pos[len(pos)-len(numbers):,1],c='black') #only plot last ones
    plt.title('tricountour interpolated contour and the directional coupler algorithm trajectory')
    plt.xlabel('Cs (stepnumber)')
    plt.ylabel('Cp (stepnumber)')
    plt.show()

def MatchingTrajectory(FolderLocation):
    Gammas = []
    numbers = []
    pathlist = Path(FolderLocation).rglob('*.LVM')
    Foldername=FolderLocation.split('/')[-1]
    for l,path in enumerate(pathlist):
        ...
        i_f = int(np.where('ICF' == np.array(data[0]['Channel names']))[0][0])
        i_r = int(np.where('ICR' == np.array(data[0]['Channel names']))[0][0])
        i_p = int(np.where('ICPhase' == np.array(data[0]['Channel names']))[0][0])

        x = data[0]['data'][:,0] #Ik denk tijd
        BeginAvg = np.abs(x[-1] - 0.05 - x).argmin() #Dichtst bij 0.05 seconden ervoor
        _,ICF = Convert('ICF',float(np.sum(data[0]['data'][BeginAvg:-1,i_f])/(len(x)-BeginAvg)),'H')
        _,ICR = Convert('ICR',float(np.sum(data[0]['data'][BeginAvg:-1,i_r])/(len(x)-BeginAvg)),'H')
        _,ICPhase = Convert('ICPhase',float(np.sum(data[0]['data'][BeginAvg:-1,i_p])/(len(x)-BeginAvg)),'H')
        Gammas.append(ICR/ICF)

    numbers, Gammas = zip(*sorted(zip(numbers, Gammas))) #sort gammas i.f.o numbers (i.e make it the right order, now is like 73,53,...)
    plt.plot(numbers,Gammas)
    plt.xlabel('Step')
    plt.ylabel(r'$| \Gamma |$')
    plt.yscale('log')
    plt.ylim(0.005,1)
...
open_button = tk.Button(root, text="file containing capacitor positions", command=open_Ch0_file_dialog)
open_button.pack(pady=20)

selected_file_ch0_label = tk.Label(root, text="Selected File:")
selected_file_ch0_label.pack()

MatchingTraj = tk.Button(root, text="Plot Gamma i.f.o steps", command= lambda: MatchingTrajectory(ch1filepath))

CapacitorTraj = tk.Button(root, text="Plot capacitor path", command= lambda: CapacitorTrajectory(ch0filepath,ch1filepath))
