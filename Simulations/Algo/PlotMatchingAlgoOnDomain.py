...
CsVals = np.linspace(150e-12,175e-12,125)#capacitor is defined in steps
CpVals = np.linspace(500e-12,750e-12,200)#capacitor is defined in steps
...
def Algo3V(Solution,probeindexA,probeindexB,probeindexC):
    ######################################
    # Calculate new values of Cs and Cp  #
    ######################################
...
def Algo2V(Solution):
    ...
    EpsB = (V[0] - Vf)*S[1] - (V[1] - Vf)*S[0]
    EpsG = (V[0] - Vf)*C[1] - (V[1] - Vf)*C[0]
...
with alive_bar(len(CsVals)*len(CpVals),title="Making colormap",length=20,bar="filling",spinner="dots_waves2") as bar:
    for j,CsVal in enumerate(CsVals):
        for k,CpVal in enumerate(CpVals):
            ...
            epsg,epsb = Algo3V(Solution,3,2,0)
    ...
    colorbar.set_label('$\Gamma$',rotation = 0)
    plt.ylabel("Cs (pF)")
    plt.xlabel("Cp (pF)")
    plt.title(f"Ca = {round(CaVal/pF_,2)}, Freq= {FREQ/MHz_}MHz; quiver shows the 4V textor algorithm")
