...
def MatchPath(algorithm,ct,CpVal,CsVal,FREQ,ProbeIndexA=None,ProbeIndexB=None,ProbeIndexC=None,phasefactor=False):
    global admittances
    global gammas
    admittances = []
    gammas = []
    ...
    while ((i < 5000) and not matched):
        ...
        if not phasefactor: 
            ...
            CsVal += EpsG*np.cos(np.angle(Gamma))
            CpVal += EpsB*np.sin(np.angle(Gamma))
...
CS = plt.imshow(Z, cmap="bone",extent=[cpmin,cpmax,csmin,csmax],origin='lower',aspect=(cpmax-cpmin)/(csmax-csmin))
...
"""
Path = MatchPath(Algo2V,ct,CpInit,CsInit,FREQ,0,3,phasefactor=True)
