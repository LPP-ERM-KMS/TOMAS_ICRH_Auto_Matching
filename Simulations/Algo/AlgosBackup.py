...
def DirCoupler(Solution,FREQ,probeindexA=None,probeindexB=None,probeindexC=None):
    ...
    Beta = 2*np.pi*FREQ/(c)
    BetaL3 = Beta*2.585
    phi = phi - 2*BetaL3 #phase transform from V3 to load
...
def Algo3V(Solution,FREQ,probeindexA,probeindexB,probeindexC,SPFactor=1):
    ...
    print(f'3V angle = {np.angle(u + 1j*v)}')
...
def Algo4V(Solution,FREQ,ProbeIndexA=None,ProbeIndexB=None,ProbeIndexC=None):
    ...
    BigS = (S[0] - S[2])/(S[1] - S[3])
    BigC = (C[0] - C[2])/(C[1] - C[3])
    ...
    u = (1/2)*((Vs[0] - Vs[2]) - (Vs[1] - Vs[3])*BigS)/((C[1] - C[3])*(BigC - BigS))
    v = (1/2)*((Vs[0] - Vs[2]) - (Vs[1] - Vs[3])*BigC)/((S[1] - S[3])*(BigC - BigS))
