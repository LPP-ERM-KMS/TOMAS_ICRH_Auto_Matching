...
CaVal=90*pF_ #CalcCa(FREQ*(10**(-6)))*(10**(-12))

#initial values:
CsVal = 53.0*pF_ #CsVals[100]
CpVal = 7.94438353*pF_ #CpVals[100]
CsFactor = pF_
CpFactor = pF_
...
while not Converged:
    ct.set('Cs.Cs',CsVal)
    ct.set('Cp.Cs',CpVal)
    Solution = ct.Solution(f=FREQ, E={'Source': np.sqrt(2*50*6*1E3)}, nodes=['V0toV1','V2toV3'])
    ...
    Vf = abs(Solution['V0toV1.V0'][2])
    Vr = abs(Solution['V0toV1.V0'][3])
