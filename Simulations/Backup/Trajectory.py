...
axs[0,0].plot(Route[:,0],Route[:,1],color="blue",lw="4")

Route = FindRoute(InitialPoint,V0V2Errors)
axs[1,0].plot(Route[:,0],Route[:,1],color="blue",lw="4")

Route = FindRoute(InitialPoint,V1V3Errors)
axs[1,1].plot(Route[:,0],Route[:,1],color="blue",lw="4")

Route = FindRoute(InitialPoint,V1V2Errors)
axs[0,1].plot(Route[:,0],Route[:,1],color="blue",lw="4")

Route = FindRoute(InitialPoint,V0V3Errors)
axs[1,2].plot(Route[:,0],Route[:,1],color="blue",lw="4")

Route = FindRoute(InitialPoint,V2V3Errors)
axs[0,2].plot(Route[:,0],Route[:,1],color="blue",lw="4")

"""
#plt.title(f"freq = {f/1e6}MHz")
fig.text(0.5, 0.04, '$\Re(\Gamma)$', ha='center')
fig.text(0.04, 0.5, '$\Im(\Gamma)$', va='center', rotation='vertical')
fig.suptitle(f'TEXTOR algorithm at {int(f/1e6)}MHz', fontsize=16)
plt.show()
